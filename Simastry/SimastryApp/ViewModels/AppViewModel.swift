...
class AppViewModel {
    ...
    var suggestedPromptsEnabled: Bool = UserDefaults.standard.object(forKey: "simastry_suggested_prompts_enabled") == nil ? true : UserDefaults.standard.bool(forKey: "simastry_suggested_prompts_enabled") {
        didSet {
            UserDefaults.standard.set(suggestedPromptsEnabled, forKey: "simastry_suggested_prompts_enabled")
        }
    }
    var connectedUsers: [SearchableUserProfile] = []
    ...
    func presentUpgradePrompt(title: String, subtitle: String) {
        guard isRevenueCatAvailable else {
            showToast(title, subtitle: "Paid plans are not available right now.", isError: true)
            return
        }
        showToast(title, subtitle: subtitle, isError: true)
        showUpsell = true
    }
    ...
    func searchUserProfiles(matching query: String) async throws -> [SearchableUserProfile] {
        let profiles = try await supabase.searchPublicProfiles(query: query)
        let connectedIds = Set(connectedUsers.map(\.id))
        return profiles.filter { !connectedIds.contains($0.id) }
    }

    func isUserConnected(_ profile: SearchableUserProfile) -> Bool {
        connectedUsers.contains { $0.id == profile.id }
    }

    func loadConnectedUsers() async {
        do {
            connectedUsers = try await supabase.fetchConnectedProfiles()
        } catch {
            showToast("Couldn't load people", subtitle: "Pull to refresh and try again.", isError: true)
        }
    }

    func addUserConnection(_ profile: SearchableUserProfile) async {
        guard !isUserConnected(profile) else { return }
        connectedUsers.insert(profile, at: 0)
        do {
            try await supabase.addUserConnection(profileId: profile.id)
            showToast("Added \(profile.displayName)", subtitle: "@\(profile.username) is now in Messages", isError: false)
            await loadConnectedUsers()
        } catch {
            connectedUsers.removeAll { $0.id == profile.id }
            showToast("Couldn't add \(profile.displayName)", subtitle: "They may have turned off discovery.", isError: true)
        }
    }

    func removeUserConnection(_ profile: SearchableUserProfile) async {
        let previousUsers = connectedUsers
        connectedUsers.removeAll { $0.id == profile.id }
        do {
            try await supabase.removeUserConnection(profileId: profile.id)
            showToast("Removed \(profile.displayName)", subtitle: "You can add @\(profile.username) again anytime", isError: false)
        } catch {
            connectedUsers = previousUsers
            showToast("Couldn't remove \(profile.displayName)", subtitle: "Try again in a moment.", isError: true)
        }
    }

    func startPrediction(for companion: CompanionData) {
        let sun = ZodiacSign(rawValue: companion.sunSign)
        let moon = ZodiacSign(rawValue: companion.moonSign)
        let rising = ZodiacSign(rawValue: companion.risingSign)
        let hint = sun.flatMap { CommunicationTemplates.guides[$0]?.bestApproach }

        predictionDraft = PredictionDraft(
            targetName: companion.name,
            targetSunSign: sun,
            targetMoonSign: moon,
            targetRisingSign: rising,
            question: nil,
            avatarURL: nil,
            communicationHint: hint
        )
        selectedTab = AppTab.predict.rawValue
    }

    func startPrediction(for profile: SearchableUserProfile, question: String? = nil) {
        predictionDraft = PredictionDraft(
            targetName: profile.displayName,
            targetSunSign: profile.sunSign,
            targetMoonSign: profile.moonSign,
            targetRisingSign: profile.risingSign,
            question: question,
            avatarURL: profile.avatarURL,
            communicationHint: profile.communicationHint
        )
        selectedTab = AppTab.predict.rawValue
    }

    func startGuidePrompt(for sign: ZodiacSign, question: String) {
        predictionDraft = PredictionDraft(
            targetName: "\(sign.displayName) guide",
            targetSunSign: sign,
            targetMoonSign: nil,
            targetRisingSign: nil,
            question: question,
            avatarURL: nil,
            communicationHint: CommunicationTemplates.guides[sign]?.bestApproach
        )
        selectedTab = AppTab.predict.rawValue
    }
    ...
    func restorePurchases() async {
        guard isRevenueCatAvailable else {
            showToast("Subscriptions unavailable", subtitle: "Paid plans are unavailable in this build.", isError: true)
            return
        }
    ...
    func createCompanion() async {
        ...
        guard canAddCompanion() else {
            presentUpgradePrompt(
                ...
                subtitle: "Upgrade your plan to add more companions"
            )
            return
        }

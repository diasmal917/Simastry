...
nonisolated enum AppScreen: Sendable {
    ...
    /// The setup flow: goal → optional chart → support style → companion.
    /// Age attestation is a sheet over the welcome screen, not a screen.
    case onboarding
...
nonisolated enum RelationshipLevel: Int, Sendable {
    ...
    static func from(messageCount: Int) -> RelationshipLevel {
        if messageCount >= 800 { return .deepTrust }
